...
<ParamField path="STRIX_REASONING_EFFORT" default="low" type="string">
...
default) inherits a bounded copy of its parent's history, so unbounded agent
fan-out is the biggest single-target token-cost driver. These caps bound it. Set
any of them to `0` to disable that limit and restore the old unbounded behavior.

<ParamField path="STRIX_MAX_AGENTS" default="4" type="integer">
...
<ParamField path="STRIX_MAX_AGENT_DEPTH" default="2" type="integer">
  Maximum spawn depth. The root is depth 1; a child of the root is depth 2. `0` = unlimited.
</ParamField>

<ParamField path="STRIX_INHERIT_CONTEXT_MAX_TOKENS" default="6000" type="integer">
...
<ParamField path="STRIX_CAVEMAN" default="ultra" type="string">
  Agent output-verbosity style, injected into **every** agent's system prompt (the root orchestrator and every sub-agent it spawns). Values: `off`, `lite`, `full`, `ultra`. When enabled, agents emit ultra-terse prose/reasoning to cut output tokens; payloads, code, commands, and vulnerability-report fields are always kept verbatim so finding accuracy is never traded for brevity. Set `off` to disable.
</ParamField>
